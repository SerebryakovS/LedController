...
bool DisplayLogoPatternCenter(RGBMatrix *Matrix, RGBMatrix::Options *MatrixOptions, FrameCanvas *OffscreenCanvas, const std::string &LogoType) {
    ...
    if (LogoType == "parqour") {
        ...
        Color1 = {  0,    0,   0};
        Color2 = {  0,  255, 145};
        LogoPattern = LogoParqour;
    } else if (LogoType == "damu") {
        Color0 = {139,  195,  74};
        Color1 = {  0,    0,   0};
        Color2 = {255,  235,  59};
        LogoPattern = LogoDamu;
    } else if (LogoType == "acl") {
        Color0 = {  0,    0,   0};
        Color1 = {255,  255,   0};
        Color2 = {255,  255, 255};
        LogoPattern = LogoACL;
    ...
    for (int Y = 0; Y < 64; ++Y) {
        for (int X = 0; X < 64; ++X) {
            if (LogoPattern[Y][X] == 0) {
                OffscreenCanvas->SetPixel(X + XOffset, Y + YOffset, Color0.Red, Color0.Green, Color0.Blue);
            } else if (LogoPattern[Y][X] == 1) {
                OffscreenCanvas->SetPixel(X + XOffset, Y + YOffset, Color1.Red, Color1.Green, Color1.Blue);
            } else if (LogoPattern[Y][X] == 2) {
                OffscreenCanvas->SetPixel(X + XOffset, Y + YOffset, Color2.Red, Color2.Green, Color2.Blue);
            } else {
    ...
    // RGB Color0 = {0,  0, 0};
    // RGB Color1 = {255, 255, 0};
    // RGB Color2 = {255,  255, 255};
    //
    // for (int Y = 0; Y < 64; ++Y) {
    //     for (int X = 0; X < 64; ++X) {
    //         if (LogoACL[Y][X] == 1) {
    //             OffscreenCanvas->SetPixel(X, Y, Color1.Red, Color1.Green, Color1.Blue);
    //         } else if (LogoACL[Y][X] == 0) {
    //             OffscreenCanvas->SetPixel(X, Y, Color0.Red, Color0.Green, Color0.Blue);
    //         } else if (LogoACL[Y][X] == 2) {
    //             OffscreenCanvas->SetPixel(X, Y, Color2.Red, Color2.Green, Color2.Blue);
    //         } else {
    //             OffscreenCanvas->SetPixel(X, Y, 0, 0, 0);
    //         };
    //     };
